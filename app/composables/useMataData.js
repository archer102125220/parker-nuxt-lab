export function useHeadMataData({
  ...
  name: _metaDataName = '',
  card: _metaDataCard = 'summary_large_image', // summary, summary_large_image, app or player
  ...
  const metaDataLanguage = computed(
    () => _metaDataLanguage || $i18n?.locale?.value || 'zh'
  );
  const metaDataImage = computed(
    () => _metaDataImage || DOMAIN + '/img/ico/web-app-manifest-512x512.png'
  );
  const metaDataName = computed(
    () => _metaDataName || $i18n.t('system.systemName')
  );
  ...
  const head = useHead({
    ...
    meta: [
      ...
      },
      {
        ...
        content: metaDataDescription.value
      },
      {
      ...
      },
      {
        ...
        content: metaDataCard.value
      },
      {
        property: 'twitter:title',
        content: metaDataTitle.value
      },
      {
