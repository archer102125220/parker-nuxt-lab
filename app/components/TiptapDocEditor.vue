...
const emits = defineEmits([
  ...
  'tiptapOnDrop',
  'tiptapOnDelete',
  'tiptapOnContentError'
]);

// 編輯器 UI 狀態。這些 ref 只影響 POC 畫面，不會直接寫進 Word 檔。
const activeUserName = ref('Parker');
const newUserName = ref('');
const editorStatus = ref('準備就緒');
const importMessages = ref([]);
const revisionEntries = ref([]);
const docxInputElement = ref(null);
...
const selectedHighlight = ref('#fef3c7');
const isReadOnlyMode = ref(false);
const revisionSerial = ref(0);

// 工具列選項先用固定陣列示範；正式產品可改成後端設定或設計系統常數。
const userNames = ref(['Parker', 'Reviewer A', 'Reviewer B']);
const fontFamilies = [
  'Arial',
  'Times New Roman',
  'Noto Sans TC',
...
const canUndo = computed(() => editor.value?.can().undo() === true);
const canRedo = computed(() => editor.value?.can().redo() === true);
...
function getToolButtonClass(key) {
  return `tiptap_doc_editor-tool_button_${key}`;
}

function getHistoryItemClass(id) {
  return `tiptap_doc_editor-history_item_${id}`;
}

function getImportMessageClass(id) {
...
function recordRevision(actionName) {
...
async function importDocxFile(event) {
...
async function insertImageFile(event) {
...
function htmlNodesToDocxChildren(elements, listLevel) {
  return elements.flatMap((element) =>
    htmlElementToDocxChildren(element, listLevel)
  );
}

function htmlElementToDocxChildren(element, listLevel) {
...
function htmlOrderedListToDocxChildren(element, listLevel) {
...
function htmlUnorderedListToDocxChildren(element, listLevel) {
...
function htmlTableToDocxTable(element) {
...
function htmlInlineChildrenToRuns(element) {
  return htmlInlineNodesToRuns(Array.from(element.childNodes));
}

function htmlInlineNodesToRuns(nodes) {
...
function htmlNodeToRuns(node, inheritedOptions) {
...
function htmlImageElementToRun(element) {
