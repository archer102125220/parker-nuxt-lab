  ...
  return new Promise((resolve, rejects) => {
    ...
    if (
      ...
      if (tryCount < tryLimit) {
        resolve(createdServerExportButtonPlugin(tryLimit, tryCount + 1));
      }
     ...
     * ```
     */
    class ServerExportButtonPlugin extends Plugin {
      static pluginName = 'server-export-plugin';
      ...
      onStarting() {
            ...
            try {
              messageService.show({
                ...
                content: localeService.t(
                  'parker-nuxt-lab-plugins.server-export.info'
                )
              });

              const startEvent = new CustomEvent(
                'univer-server-export-started',
                {
                  bubbles: true
                }
              );
              ...
              if (isDoc === true) {
                ...
                throw new Error(
                  localeService.t(
                    'parker-nuxt-lab-plugins.server-export.error.snapshot'
                  )
                );
              }
              ...
              if (uploadRes.ok === false) {
                ...
                throw new Error(
                  `${localeService.t('parker-nuxt-lab-plugins.server-export.error.uploadFailed')} (${uploadRes.status}): ${errText}`
                );
              }
              ...
              if (
                ...
                throw new Error(
                  localeService.t(
                    'parker-nuxt-lab-plugins.server-export.error.uploadSnapshotFailed'
                  )
                );
              }
              ...
              if (typeof taskID !== 'string' || taskID === '') {
                throw new Error(
                  localeService.t(
                    'parker-nuxt-lab-plugins.server-export.error.taskFailed'
                  )
                );
              }
                ...
                if (
                  ...
                  throw new Error(
                    taskData.error?.message ||
                      localeService.t(
                        'parker-nuxt-lab-plugins.server-export.error.backendTaskFailed'
              ...
              if (isSuccess === false || finalTaskData === null) {
                throw new Error(
                  localeService.t(
                    'parker-nuxt-lab-plugins.server-export.error.timeout'
                  )
                );
              }
              ...
              messageService.show({
                ...
                content:
                  localeService.t(
                    'parker-nuxt-lab-plugins.server-export.error.exportFailed'
                  ) + errorMessage
              });
              return false;
            } finally {
        ...
        const menuItemFactory = () => ({
          id: buttonId,
          title: 'parker-nuxt-lab-plugins.server-export.title',
          tooltip: 'parker-nuxt-lab-plugins.server-export.tooltip',
    ...
    resolve(ServerExportButtonPlugin);
  });
}

export default createdServerExportButtonPlugin;
