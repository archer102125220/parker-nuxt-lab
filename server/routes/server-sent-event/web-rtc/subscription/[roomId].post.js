    ...
    const interval = setInterval(async function() {
        try {
            const [memberTypeString, memberCandidateListString, memberDescriptionListString] = await Promise.all([
                UpstashRedis.get(`nuxt-lab:web-rtc-member-type-${roomId}-${userId}`),
                UpstashRedis.get(`nuxt-lab:web-rtc-member-candidate-list-${roomId}`),
                UpstashRedis.get(`nuxt-lab:web-rtc-member-description-list-${roomId}`),
            ]);
