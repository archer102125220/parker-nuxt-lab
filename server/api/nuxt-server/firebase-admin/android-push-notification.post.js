...
/**
 * @openapi
 * /nuxt-server/firebase-admin/android-push-notification:
 *    post:
 *      description: Android 推播通知
 *      requestBody:
 *        required: true
 *        content:
 *          application/json:
 ...
 *                token:
 *                  type: array
 *                  description: Android 裝置的 FCM 權杖陣列
 *                  items:
 *                    type: string
 *                  example: ["android_token_1", "android_token_2"]
 *      responses:
 *        200:
 ...
 *
 * 發送推播通知到 Android 裝置
 * 使用 Firebase Cloud Messaging (FCM) 向指定的 Android 裝置推送訊息
 *
 * @api {POST} /api/nuxt-server/firebase-admin/android-push-notification Android 推播通知
 * @apiGroup Firebase
 * @apiName AndroidPushNotification
 *
 * @apiBody {String} data 推播訊息內容
 * @apiBody {String} [title] 推播訊息標題
 * @apiBody {String} [img] 推播訊息圖片 URL
 * @apiBody {Array<String>} token Android 裝置的 FCM 權杖陣列
 *
 ...
 * @example
 * // 請求範例
 * POST /api/nuxt-server/firebase-admin/android-push-notification
 * Content-Type: application/json
 * {
 *   "data": "您有新的訊息",
 *   "title": "通知標題",
 *   "img": "https://example.com/image.jpg",
 *   "token": ["android_token_1", "android_token_2"]
 * }
 *
 * @example
 * // 成功回應範例
 ...
 *
 * @description
 * 此 API 專門用於向 Android 裝置發送推播通知：
 *
 * 1. 驗證必要的參數 (data 和 token)
 * 2. 使用 Firebase Admin Android 應用實例
